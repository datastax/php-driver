...
PHP_METHOD(TypeSet, type)
{
  ...
  self = PHP_CASSANDRA_GET_TYPE_SET(getThis());
...
{
  cassandra_type_set *self = PHP5TO7_ZEND_OBJECT_GET(type_set, object);
...
{
  cassandra_type_set *self =
      PHP5TO7_ZEND_OBJECT_ECALLOC(type_set, ce);

  self->type = CASS_VALUE_TYPE_UNKNOWN;

  PHP5TO7_ZEND_OBJECT_INIT(type_set, self, ce);
}
