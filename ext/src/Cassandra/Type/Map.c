...
PHP_METHOD(TypeMap, keyType)
{
  ...
  self = PHP_CASSANDRA_GET_TYPE_MAP(getThis());
  ztype = php_cassandra_type_scalar(self->key_type TSRMLS_CC);
...
PHP_METHOD(TypeMap, valueType)
{
  ...
  self = PHP_CASSANDRA_GET_TYPE_MAP(getThis());
  ztype = php_cassandra_type_scalar(self->value_type TSRMLS_CC);
  RETURN_ZVAL(PHP5TO7_ZVAL_MAYBE_P(ztype), 1, 0);
}
...
{
  cassandra_type_map *self =
      PHP5TO7_ZEND_OBJECT_ECALLOC(type_map, ce);

  self->key_type = CASS_VALUE_TYPE_UNKNOWN;
  self->value_type = CASS_VALUE_TYPE_UNKNOWN;

  PHP5TO7_ZEND_OBJECT_INIT(type_map, self, ce);
}
