...
static enum token_type ip_address_tokenize(char *address, char *token, int *token_len, char **next_token)
{
...
int php_driver_parse_ip_address(char *in, CassInet *inet )
{
    ...
    else if (!domain)
    {
        ...
        {
            zend_throw_exception_ex(php_driver_invalid_argument_exception_ce, 0 ,
                                    "Address \"%s\" contains only %d bytes  (%d bytes are expected)", in, pos + 1,
                                    CASS_INET_V6_LENGTH);
            return 0;
        }
...
void php_driver_format_address(CassInet inet, char **out)
{
    if (inet.address_length > 4)
        spprintf(out, 0, "%x:%x:%x:%x:%x:%x:%x:%x", (inet.address[0] * 256 + inet.address[1]),
                 ...
                 (inet.address[10] * 256 + inet.address[11]), (inet.address[12] * 256 + inet.address[13]),
                 (inet.address[14] * 256 + inet.address[15]));
    else
        spprintf(out, 0, "%d.%d.%d.%d", inet.address[0], inet.address[1], inet.address[2], inet.address[3]);
}
