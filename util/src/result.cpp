    ...
    case CASS_VALUE_TYPE_UDT:
        ...
        while (cass_iterator_next(iterator))
        {
            ...
            if (!cass_value_is_null(value))
            {
                ...
                php_driver_user_type_value_set(user_type_value, name, name_length, PHP5TO7_ZVAL_MAYBE_P(v) TSRMLS_CC);
                zval_ptr_dtor(&v);
            }
...
int php_driver_get_result(const CassResult *result, php5to7_zval *out TSRMLS_DC)
{
    ...
    while (cass_iterator_next(iterator))
    {
        ...
        for (i = 0; i < columns; i++)
        {
            ...
            PHP5TO7_ADD_ASSOC_ZVAL_EX(PHP5TO7_ZVAL_MAYBE_P(row), column_names[i], strlen(column_names[i]) + 1,
                                      PHP5TO7_ZVAL_MAYBE_P(value));
        }

        add_next_index_zval(PHP5TO7_ZVAL_MAYBE_P(rows), PHP5TO7_ZVAL_MAYBE_P(row));
    }
