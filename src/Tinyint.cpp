...
static zend_result to_double(zval *result, php_driver_numeric *tinyint TSRMLS_DC)
{
    ZVAL_DOUBLE(result, (double)tinyint->data.tinyint.value);
    return SUCCESS;
}

static zend_result to_long(zval *result, php_driver_numeric *tinyint TSRMLS_DC)
{
    ZVAL_LONG(result, (php5to7_long)tinyint->data.tinyint.value);
    return SUCCESS;
}

static zend_result to_string(zval *result, php_driver_numeric *tinyint TSRMLS_DC)
{
    char *string;
...
void php_driver_tinyint_init(INTERNAL_FUNCTION_PARAMETERS)
{
    ...
    if (Z_TYPE_P(value) == IS_OBJECT && instanceof_function(Z_OBJCE_P(value), php_driver_tinyint_ce TSRMLS_CC))
    {
...
PHP_METHOD(Tinyint, add)
{
    ...
    if (Z_TYPE_P(addend) == IS_OBJECT && instanceof_function(Z_OBJCE_P(addend), php_driver_tinyint_ce TSRMLS_CC))
    {
        self = PHP_DRIVER_GET_NUMERIC(getThis());
...
PHP_METHOD(Tinyint, div)
{
    ...
    if (Z_TYPE_P(divisor) == IS_OBJECT && instanceof_function(Z_OBJCE_P(divisor), php_driver_tinyint_ce TSRMLS_CC))
    {
        php_driver_numeric *self = PHP_DRIVER_GET_NUMERIC(getThis());
...
static int php_driver_tinyint_compare(zval *obj1, zval *obj2 TSRMLS_DC)
{
#if PHP_MAJOR_VERSION >= 8
...
static unsigned php_driver_tinyint_hash_value(zval *obj TSRMLS_DC)
{
...
static
    ...
    int
#endif
    php_driver_tinyint_cast(zend_object *object, zval *retval, int type TSRMLS_DC)
{
#if PHP_MAJOR_VERSION >= 8
    php_driver_numeric *self = PHP5TO7_ZEND_OBJECT_GET(numeric, object);
#else
...
static void php_driver_tinyint_free(php5to7_zend_object_free *object TSRMLS_DC)
{
    php_driver_numeric *self = PHP5TO7_ZEND_OBJECT_GET(numeric, object);
...
static php5to7_zend_object php_driver_tinyint_new(zend_class_entry *ce TSRMLS_DC)
{
