...
static zend_result to_double(zval *result, php_driver_numeric *bigint TSRMLS_DC)
{
    ZVAL_DOUBLE(result, (double)bigint->data.bigint.value);
    return SUCCESS;
}

static zend_result to_long(zval *result, php_driver_numeric *bigint TSRMLS_DC)
{
...
static zend_result to_string(zval *result, php_driver_numeric *bigint TSRMLS_DC)
{
    char *string;
...
void php_driver_bigint_init(INTERNAL_FUNCTION_PARAMETERS)
{
    ...
    {
        if (!php_driver_parse_bigint(Z_STRVAL_P(value), Z_STRLEN_P(value), &self->data.bigint.value TSRMLS_CC))
        {
            return;
        }
    }
    else if (Z_TYPE_P(value) == IS_OBJECT && instanceof_function(Z_OBJCE_P(value), php_driver_bigint_ce TSRMLS_CC))
    {
...
PHP_METHOD(Bigint, add)
{
    ...
    if (Z_TYPE_P(num) == IS_OBJECT && instanceof_function(Z_OBJCE_P(num), php_driver_bigint_ce TSRMLS_CC))
    {
        self = PHP_DRIVER_GET_NUMERIC(getThis());
...
static int php_driver_bigint_compare(zval *obj1, zval *obj2 TSRMLS_DC)
{
#if PHP_MAJOR_VERSION >= 8
...
static unsigned php_driver_bigint_hash_value(zval *obj TSRMLS_DC)
{
...
static void php_driver_bigint_free(php5to7_zend_object_free *object TSRMLS_DC)
{
    php_driver_numeric *self = PHP5TO7_ZEND_OBJECT_GET(numeric, object);
...
static php5to7_zend_object php_driver_bigint_new(zend_class_entry *ce TSRMLS_DC)
{
