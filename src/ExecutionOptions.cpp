...
static zend_result build_from_array(php_driver_execution_options *self, zval *options, int copy)
{
    ...
    {
        if (Z_TYPE_P(PHP5TO7_ZVAL_MAYBE_DEREF(page_size)) != IS_LONG ||
            Z_LVAL_P(PHP5TO7_ZVAL_MAYBE_DEREF(page_size)) <= 0)
        {
    ...
    {
        if (!(Z_TYPE_P(PHP5TO7_ZVAL_MAYBE_DEREF(timeout)) == IS_LONG &&
              Z_LVAL_P(PHP5TO7_ZVAL_MAYBE_DEREF(timeout)) > 0) &&
            !(Z_TYPE_P(PHP5TO7_ZVAL_MAYBE_DEREF(timeout)) == IS_DOUBLE &&
              Z_DVAL_P(PHP5TO7_ZVAL_MAYBE_DEREF(timeout)) > 0) &&
            !(Z_TYPE_P(PHP5TO7_ZVAL_MAYBE_DEREF(timeout)) == IS_NULL))
        {
            throw_invalid_argument(PHP5TO7_ZVAL_MAYBE_DEREF(timeout), "timeout",
    ...
    if (PHP5TO7_ZEND_HASH_FIND(Z_ARRVAL_P(options), "retry_policy", sizeof("retry_policy"), retry_policy))
    {
        if (Z_TYPE_P(PHP5TO7_ZVAL_MAYBE_DEREF(retry_policy)) != IS_OBJECT &&
            !instanceof_function(Z_OBJCE_P(PHP5TO7_ZVAL_MAYBE_DEREF(retry_policy)), php_driver_retry_policy_ce))
        {
            throw_invalid_argument(PHP5TO7_ZVAL_MAYBE_DEREF(retry_policy), "retry_policy",
    ...
    if (PHP5TO7_ZEND_HASH_FIND(Z_ARRVAL_P(options), "timestamp", sizeof("timestamp"), timestamp))
    {
        ...
        else if (Z_TYPE_P(PHP5TO7_ZVAL_MAYBE_DEREF(timestamp)) == IS_STRING)
        {
            if (!php_driver_parse_bigint(Z_STRVAL_P(PHP5TO7_ZVAL_MAYBE_DEREF(timestamp)),
                                         Z_STRLEN_P(PHP5TO7_ZVAL_MAYBE_DEREF(timestamp)), &self->timestamp))
            {
                return FAILURE;
            }
        }
...
PHP_METHOD(ExecutionOptions, __get)
{
    ...
    else if (name_len == 16 && strncmp("pagingStateToken", name, name_len) == 0)
    {
        ...
        PHP5TO7_RETURN_STRINGL(self->paging_state_token, self->paging_state_token_size);
    }
